	...
	for (let i = 0; i < result.length; i++) {
		...
		if (moviedata.Response) {	
			...
			if (pageType == "info") {	
				...
				result[i].Rated = moviedata.Rated;
...
function createMovieCard(moviedata) {
	...
	// this is creating a div with the content inside of it
					content = 
					`<div id="`+ moviedata.imdbID +`"class="moviecards col-sm-4 card border-secondary sm-3" style="max-width: 20rem; min-width: 20rem; align-items: center; border-color: #3b7cd6 !important; border-width: 3px; margin-top: 20px;" onmouseover="movieHoverIn(this)" onmouseout="movieHoverOut(this)" onclick="loadInfo('`+ moviedata.imdbID +`','`+moviedata.Year+`')">
